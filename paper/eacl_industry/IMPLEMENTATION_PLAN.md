...
| Evidence graph | Rule-based nodes and edges order and budget retrieved context, then appear in traces/UI. | It has no effectiveness ablation and is presented only as a context-selection audit, not a reasoning improvement. |
...
| API | `/chat` and `/chat/stream` call the same `AnswerPipelineService`; the latter explicitly emits staged trace events and one completed-answer event. | Shared production/evaluation path is a strength. Do not claim token streaming or time-to-first-token. |
