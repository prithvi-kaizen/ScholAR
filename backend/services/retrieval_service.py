...
def retrieve_chunks(message: str, chunks: list[dict[str, Any]], limit: int = 4, preferred_pages: list[int] | None = None) -> list[dict[str, Any]]:
    ...
    candidates = bm25_ranked[: max(limit * 8, 20)]
    ...
    for rank, (bm25_score, chunk) in enumerate(candidates):
        ...
        lowered = text.lower()
        tokens = tokenize(text)
        semantic = _cosine(query_vector, _hashed_embedding(tokens))
